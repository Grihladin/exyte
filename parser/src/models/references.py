...
class Reference(BaseModel):
    ...
    type: str = Field(
        ..., description="Type of reference (section, table, figure, external)"
    )
...
class TableData(BaseModel):
    ...
    accuracy: Optional[float] = Field(
        None, description="Extraction accuracy score (0-100)"
    )
    ...
    table_info: list[str] = Field(
        default_factory=list,
        description="Notes and additional info below the table (For SI:, a., b., etc.)",
    )
    table_name: str = Field(
        ...,
        description="Table title/name with TABLE [NUMBER] prefix (e.g., 'TABLE 719.1 MULTIPLYING FACTOR FOR FINISHES')",
    )
...
class TableReference(Reference):
    ...
    table_data: Optional[TableData] = Field(
        None, description="Extracted table structure and content"
    )
...
class FigureReference(Reference):
    ...
    dimensions: Optional[dict[str, int]] = Field(
        None, description="Image dimensions (width, height)"
    )
...
class References(BaseModel):
    ...
    external_documents: list[ExternalDocumentReference] = Field(
        default_factory=list, description="External document references"
    )
